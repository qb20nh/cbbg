        ...
        context.runOnClient(
            client -> {
              TextureTarget external =
                  new TextureTarget("External framebuffer", 16, 16, false, GpuFormat.RGBA8_UNORM);
              try {
                external.resize(32, 24);
                if (!hasFormat(external.getColorTexture(), GpuFormat.RGBA8_UNORM, GL11.GL_RGBA8)) {
    ...
    try {
      ...
      for (String line : lines) {
        ...
        if (!Files.isRegularFile(image)
            || !HexFormat.of().formatHex(digest.digest(Files.readAllBytes(image))).equals(parts[0]))
          return false;
      }
      return true;
    } catch (Exception e) {
