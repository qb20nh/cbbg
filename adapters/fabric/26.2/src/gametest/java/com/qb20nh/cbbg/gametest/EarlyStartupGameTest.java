...
public final class EarlyStartupGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context) {
        ...
        if (Boolean.getBoolean("cbbg.test.lifecycle")) {
            try (var world = context.worldBuilder().create()) {
                ...
                context.waitFor(client -> isRgba32f(
                        client.gameRenderer.mainRenderTarget().getColorTexture()), 600);
            }
        }
        try {
    ...
    private static boolean isRgba32f(GpuTexture texture) {
        if (!(texture instanceof GlTexture gl)) return texture.getFormat() == GpuFormat.RGBA32_FLOAT;
        int previous = GL11.glGetInteger(GL11.GL_TEXTURE_BINDING_2D);
        try {
            GL11.glBindTexture(GL11.GL_TEXTURE_2D, gl.glId());
            return GL11.glGetTexLevelParameteri(GL11.GL_TEXTURE_2D, 0,
                    GL11.GL_TEXTURE_INTERNAL_FORMAT) == GL30.GL_RGBA32F;
        } finally {
