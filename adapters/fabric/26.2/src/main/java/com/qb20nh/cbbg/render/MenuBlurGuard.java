 ...
 *
 * <p>Vanilla runs the blur as a post-processing chain during GUI rendering (see {@code
 * GameRenderer#processBlurEffect}). The post chain allocates intermediate {@code RenderTarget}s via
 * {@code TextureTarget(null, ...)} which default to RGBA8.
 *
...
public final class MenuBlurGuard {
   ...
   *
   * <p>This is separate from {@link #isActive()} so the caller can track transitions across frames
