...
public class STBNGenerator {
    ...
    public record STBNFields(double[] uField, double[] vField) {
        ...
        public boolean equals(Object o) {
            return this == o || o instanceof STBNFields that
                    && Arrays.equals(uField, that.uField) && Arrays.equals(vField, that.vField);
        }
    ...
    public static synchronized CompletableFuture<STBNFields> generateAsync(int w, int h, int d,
            long seed, boolean force) {
        long start = System.nanoTime();
        CompletableFuture<NoisePreparation.Fields> next = preparation.prepare(w, h, d, seed,
                force, () -> {
        ...
        if (next != preparationFuture) {
            ...
            pendingFuture = next.thenApply(fields -> {
                LOGGER.info("STBN preparation complete in {} ms ({})",
                        (System.nanoTime() - start) / 1_000_000, fields == null ? "cache" : "generated");
