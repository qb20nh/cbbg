...
public final class MainTargetFormatSupport {
  ...
  public static CbbgConfig.PixelFormat getEffective(CbbgConfig.PixelFormat requested) {
    ...
    return switch (requested) {
      ...
      case RGBA16F ->
          isSupported(CbbgConfig.PixelFormat.RGBA16F)
  ...
  private static boolean isRgba16fSupported() {
    ...
    if (state == SupportState.UNKNOWN) {
      rgba16f =
          probeColorRenderable(CbbgConfig.PixelFormat.RGBA16F)
  ...
  private static boolean isRgba32fSupported() {
    ...
    if (state == SupportState.UNKNOWN) {
      rgba32f =
          probeColorRenderable(CbbgConfig.PixelFormat.RGBA32F)
              ? SupportState.SUPPORTED
              : SupportState.UNSUPPORTED;
      state = rgba32f;
      if (state == SupportState.UNSUPPORTED) {
  ...
  private static void disableRgba16f(@Nullable Throwable cause) {
    ...
    if (cause == null) {
      ...
      Cbbg.LOGGER.warn(
          "RGBA16F main render target failed; disabling for this session and falling back.", cause);
    }
  ...
  private static void disableRgba32f(@Nullable Throwable cause) {
    ...
    if (cause == null) {
      ...
      Cbbg.LOGGER.warn(
          "RGBA32F main render target failed; disabling for this session and falling back.", cause);
    }
    logNoFloatFormatsIfApplicable();
  }
  ...
  private static boolean probeColorRenderable(CbbgConfig.PixelFormat format) {
    ...
    if (!isOpenGl()) {
      try (GpuTexture ignored =
          RenderSystem.getDevice()
              .createTexture("cbbg format probe", 15, toGpuFormat(format), 16, 16, 1, 1)) {
        return true;
      } catch (Exception e) {
    ...
    int internalFormat =
        format == CbbgConfig.PixelFormat.RGBA32F ? GL30.GL_RGBA32F : GL30.GL_RGBA16F;
    ...
    try {
      ...
      GL30.glFramebufferTexture2D(
          GL30.GL_FRAMEBUFFER, GL30.GL_COLOR_ATTACHMENT0, GL11.GL_TEXTURE_2D, tex, 0);
