...
public final class CbbgClientCommands<S> {
  ...
  private static final BoolArgumentType BOOL_ARG = Objects.requireNonNull(BoolArgumentType.bool());
  ...
  private void sendFeedback(S source, Component message) {
    feedback.accept(source, message);
  }

  private void sendError(S source, Component message) {
    error.accept(source, message);
  }
  ...
  public static <S> void registerCommands(
      CommandDispatcher<S> dispatcher,
      Runnable resetAfterToggle,
      Consumer<Boolean> reloadStbn,
      BiConsumer<S, Component> feedback,
      BiConsumer<S, Component> error) {
    new CbbgClientCommands<S>(feedback, error)
        .registerTree(dispatcher, resetAfterToggle, reloadStbn);
  }

  private void registerTree(
      CommandDispatcher<S> dispatcher, Runnable resetAfterToggle, Consumer<Boolean> reloadStbn) {
    dispatcher.register(
        literal("cbbg")
            .executes(
                ctx -> {
            ...
            .then(modeCommand(resetAfterToggle))
            .then(formatCommand(resetAfterToggle))
  ...
  private LiteralArgumentBuilder<S> modeCommand(Runnable resetAfterToggle) {
    return literal(ARG_MODE)
        .executes(
            ctx -> {
              var mode = CbbgConfig.get().mode();
              sendFeedback(
                  ctx.getSource(), Text.translatable("cbbg.command.mode.current", modeName(mode)));
              return 1;
            })
        .then(
            literal("set")
                        ...
                        .executes(
                            ctx -> {
                              ...
                              try {
                                ...
                                sendFeedback(
                                    ctx.getSource(),
                                    Text.translatable("cbbg.command.mode.set", modeName(mode)));
                                return 1;
                              } catch (Exception e) {
                                sendError(
                                    ctx.getSource(),
  ...
  private LiteralArgumentBuilder<S> formatCommand(Runnable resetAfterToggle) {
    return literal(ARG_FORMAT)
        .executes(
            ctx -> {
              var format = CbbgConfig.get().pixelFormat();
              sendFeedback(
                  ctx.getSource(),
                  Text.translatable("cbbg.command.format.current", pixelFormatName(format)));
              return 1;
            })
        .then(
            literal("set")
                .then(
                    argument(ARG_FORMAT, STRING_ARG)
                        .suggests(
                            (ctx, builder) -> {
                              ...
                              try {
                                ...
                                if (fmt == CbbgConfig.PixelFormat.RGBA8) {
                                  sendError(
                                      ctx.getSource(),
                                      Text.translatable(
                                          "cbbg.command.format.rgba8_not_selectable"));
                                  return 0;
                                }
                                ...
                                sendFeedback(
                                    ctx.getSource(),
                                    Text.translatable(
                                        "cbbg.command.format.set", pixelFormatName(fmt)));
                                return 1;
                              } catch (Exception e) {
                                sendError(
                                    ctx.getSource(),
    ...
    return literal("stbn")
        ...
        .then(
            literal("generate")
                .executes(
                    ctx -> {
                      sendFeedback(
                          ctx.getSource(), Text.translatable("cbbg.command.stbn.generating"));
        ...
        .then(
            literal("size")
                ...
                .then(
                    argument(ARG_VALUE, STBN_SIZE_ARG)
                        .executes(
                            ctx -> {
                              ...
                              if (Integer.bitCount(size) != 1) { // Check power of two
                                sendError(
                                    ctx.getSource(),
                                    Text.translatable("cbbg.command.stbn.size.invalid_pow2"));
                                sendStbnSizeUsage(ctx.getSource());
                                return 0;
                              }
                              ...
                              sendFeedback(
                                  ctx.getSource(),
                                  Text.translatable("cbbg.command.stbn.size.set", size));
                              return 1;
                            })))
        .then(
            literal("depth")
                ...
                .then(
                    argument(ARG_VALUE, STBN_DEPTH_ARG)
                        .executes(
                            ctx -> {
                              ...
                              if (Integer.bitCount(depth) != 1) {
                                sendError(
                                    ctx.getSource(),
                                    Text.translatable("cbbg.command.stbn.depth.invalid_pow2"));
                                sendStbnDepthUsage(ctx.getSource());
                                return 0;
                              }
                              ...
                              sendFeedback(
                                  ctx.getSource(),
                                  Text.translatable("cbbg.command.stbn.depth.set", depth));
                              return 1;
                            })))
        .then(
            literal("seed")
                ...
                .then(
                    argument(ARG_VALUE, STBN_SEED_ARG)
                        .executes(
                            ctx -> {
                              ...
                              sendFeedback(
                                  ctx.getSource(),
                                  Text.translatable("cbbg.command.stbn.seed.set", seed));
                              return 1;
                            })))
        .then(
            literal("reset")
                ...
                .executes(
                    ctx -> {
                      sendFeedback(
                          ctx.getSource(),
                          Text.translatable(
                              "cbbg.command.notification.chat.current",
                ...
                .executes(
                    ctx -> {
                      sendFeedback(
                          ctx.getSource(),
                          Text.translatable(
                              "cbbg.command.notification.toast.current",
                        ...
                        .executes(
                            ctx -> {
                              ...
                              sendFeedback(
                                  ctx.getSource(),
                                  Text.translatable(
                                      "cbbg.command.notification.toast.current", val));
                              return 1;
                            })));
  }
  ...
  private void sendStbnUsage(S source) {
    ...
    sendFeedback(
        source,
        Text.translatable(
            "cbbg.command.stbn.usage.header", cfg.stbnSize(), cfg.stbnDepth(), cfg.stbnSeed()));
  ...
  private void sendStbnSizeUsage(S source) {
    sendFeedback(
        source, Text.translatable("cbbg.command.stbn.size.current", CbbgConfig.get().stbnSize()));
    sendFeedback(source, Text.translatable("cbbg.command.stbn.size.usage"));
  }

  private void sendStbnDepthUsage(S source) {
    sendFeedback(
        source, Text.translatable("cbbg.command.stbn.depth.current", CbbgConfig.get().stbnDepth()));
    sendFeedback(source, Text.translatable("cbbg.command.stbn.depth.usage"));
  }

  private void sendStbnSeedUsage(S source) {
    sendFeedback(
        source, Text.translatable("cbbg.command.stbn.seed.current", CbbgConfig.get().stbnSeed()));
  ...
  private void sendNotificationUsage(S source) {
    ...
    sendFeedback(
        source,
        Text.translatable(
            "cbbg.command.notification.usage.header", cfg.notifyChat(), cfg.notifyToast()));
