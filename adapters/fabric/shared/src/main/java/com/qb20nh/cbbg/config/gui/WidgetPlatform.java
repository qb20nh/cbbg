...
final class WidgetPlatform {
    ...
    static <T> CycleButton.Builder<T> cycleTooltip(CycleButton.Builder<T> builder, Function<T, Component> text) {
        return builder.withTooltip(value -> Tooltip.create(text.apply(value)));
    }

    static Button button(Component label, Button.OnPress action, int x, int y, int width, int height, Component tooltip) {
    ...
    static String filterText(String text) { return StringUtil.filterText(text); }
}
