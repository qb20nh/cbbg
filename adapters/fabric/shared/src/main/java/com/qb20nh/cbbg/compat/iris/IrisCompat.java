...
public final class IrisCompat {
  ...
  private static final boolean IRIS_LOADED =
      LoaderPlatform.isModLoaded("iris") || LoaderPlatform.isModLoaded("oculus");
  private static volatile boolean loggedReflectionFailure = false;

  private IrisCompat() {}

  /**
   * Reports whether Iris is loaded and a shaderpack is currently in use.
   *
  ...
  private static void logReflectionFailureOnce(@Nullable Throwable t) {
    ...
    Cbbg.LOGGER.warn(
        "Failed to query Iris shaderpack state via reflection; disabling cbbg for safety.", t);
  }
}
