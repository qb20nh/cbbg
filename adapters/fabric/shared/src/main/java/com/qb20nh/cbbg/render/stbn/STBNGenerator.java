...
public class STBNGenerator {
  ...
  static synchronized CompletableFuture<STBNFields> prepareEarly(
  ...
  static synchronized CompletableFuture<STBNFields> generateAsync(
      int w, int h, int d, long seed, BooleanSupplier cacheValid) {
  ...
  private static void complete(
      CompletableFuture<STBNFields> future, STBNFields fields, Throwable failure) {
  ...
  private static CompletableFuture<STBNFields> submitJob(
      int w, int h, int d, long seed, BooleanSupplier cacheValid) {
    CompletableFuture<STBNFields> future = new CompletableFuture<>();
    FutureTask<STBNFields> task =
        new FutureTask<>(
            () -> {
              try {
                ...
                if (cacheValid.getAsBoolean()) {
                  LOGGER.info(
                      "Valid STBN cache found for {}x{}x{}. Skipping math generation.", w, h, d);
                  return null;
                }
