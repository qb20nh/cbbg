...
public final class DitherReference {
    ...
    public static int channel(double source, int noiseByte, double strength,
            int x, int width, boolean demo) {
        double amount = demo && x < width / 2 ? 0 : strength;
        double clamped = Math.max(0, Math.min(1, source));
        int quantized = (int) Math.floor(clamped * 255
                + (noiseByte / 255.0 - 0.5) * amount + 0.5);
