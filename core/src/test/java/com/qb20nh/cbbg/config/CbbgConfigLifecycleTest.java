...
class CbbgConfigLifecycleTest {
    ...
    void loaderInitializesOnceAndSettersPreserveOtherSettings() {
        ...
        CbbgConfig expected = new CbbgConfig(CbbgConfig.Mode.DEMO,
                CbbgConfig.PixelFormat.RGBA32F, 64, 32, 12345, 2, false, false);
        ...
        int[] calls = new int[3];
        LegacyCommandGrammar.Feedback feedback = new LegacyCommandGrammar.Feedback() {
            public Object translate(String key) { return key; }
        ...
        for (String command : new String[] {"format set rgba8", "stbn size 33",
                "stbn depth 256", "stbn seed 9223372036854775808", "notification toast true"}) {
            int errors = calls[0];
            assertTrue(LegacyCommandGrammar.execute("/cbbg " + command, false, feedback, generate, reload));
        ...
        assertFalse(LegacyCommandGrammar.execute("/other stbn generate", true, feedback, generate, reload));
        assertTrue(LegacyCommandGrammar.execute("/cbbg stbn generate", true, feedback, generate, reload));
        ...
        CbbgConfig reset = new CbbgConfig(CbbgConfig.Mode.DEMO,
                CbbgConfig.PixelFormat.RGBA32F, 128, 64, 0, 2, false, false);
