...
class FormatPolicyTest {
    ...
    void fullCapabilityMatrixPreservesRequestedPrecisionOrFallsBack() {
        for (boolean half : new boolean[] {false, true}) {
            for (boolean full : new boolean[] {false, true}) {
                ...
                assertEquals(half ? PixelFormat.RGBA16F : PixelFormat.RGBA8,
                        FormatPolicy.effective(PixelFormat.RGBA16F, supported));
                assertEquals(full ? PixelFormat.RGBA32F : half ? PixelFormat.RGBA16F : PixelFormat.RGBA8,
    ...
    void probesOnlyNeededFormatsInDescendingOrder() {
        ...
        FormatPolicy.effective(null, format -> { probes.add(format); return false; });
        FormatPolicy.effective(PixelFormat.RGBA8, format -> { probes.add(format); return false; });
        assertTrue(probes.isEmpty());
        FormatPolicy.effective(PixelFormat.RGBA32F, format -> { probes.add(format); return false; });
        assertEquals(Arrays.asList(PixelFormat.RGBA32F, PixelFormat.RGBA16F), probes);
        probes.clear();
        FormatPolicy.effective(PixelFormat.RGBA32F, format -> { probes.add(format); return true; });
