...
class NoisePreparationTest {
    ...
    void startupAndRendererReuseTheSameJob() throws Exception {
        ...
        try (NoisePreparation preparation = new NoisePreparation()) {
            CompletableFuture<NoisePreparation.Fields> first = preparation.prepare(8, 4, 2, 42,
                    false, () -> { checks.incrementAndGet(); return false; });
            ExecutorService callers = Executors.newFixedThreadPool(4);
            try {
                List<Future<CompletableFuture<NoisePreparation.Fields>>> requests = new ArrayList<>();
                for (int i = 0; i < 12; i++) {
                    requests.add(callers.submit(() -> preparation.prepare(8, 4, 2, 42, false,
                            () -> { fail("Duplicate cache check"); return false; })));
                }
    ...
    void warmCacheSkipsMathAndForceRegenerates() throws Exception {
        try (NoisePreparation preparation = new NoisePreparation()) {
            CompletableFuture<NoisePreparation.Fields> cached = preparation.prepare(8, 4, 2, 0,
                    false, () -> true);
            assertNull(cached.get(5, TimeUnit.SECONDS));
            CompletableFuture<NoisePreparation.Fields> forced = preparation.prepare(8, 4, 2, 0,
                    true, () -> { fail("Forced generation must skip the cache"); return true; });
    ...
    void changedSettingsInterruptTheOldJobAndUseTheNewSeed() throws Exception {
        ...
        try (NoisePreparation preparation = new NoisePreparation()) {
            CompletableFuture<NoisePreparation.Fields> old = preparation.prepare(8, 4, 2, 0,
                    false, () -> {
                        started.countDown();
                        try {
            ...
            CompletableFuture<NoisePreparation.Fields> next = preparation.prepare(4, 4, 2, 7,
                    false, () -> { fail("Old cache cannot identify the new seed"); return true; });
            assertTrue(interrupted.await(5, TimeUnit.SECONDS));
            assertTrue(old.isCancelled());
            assertTrue(preparation.matches(4, 4, 2, 7));
            assertFalse(preparation.matches(8, 4, 2, 0));
            assertArrayEquals(BlueNoise.generateScalarField(4, 4, 2, 7 * 31),
                    next.get(5, TimeUnit.SECONDS).u());
        }
    }
    ...
    void failuresRemainAvailableUntilExplicitRetry() throws Exception {
        try (NoisePreparation preparation = new NoisePreparation()) {
            CompletableFuture<NoisePreparation.Fields> failed = preparation.prepare(8, 4, 2, 0,
                    false, () -> { throw new IllegalStateException("cache unavailable"); });
