...
public final class LegacyCommandGrammar {
    ...
    public static List<String> suggest(String message, boolean toast) {
        ...
        if (args.length == 1) candidates.add("/cbbg");
            ...
            } else if (args[1].equals("notification") && (args[2].equals("chat")
                    || args[2].equals("toast") && toast)) {
    ...
    public static boolean execute(String message, boolean toast, Feedback feedback, Runnable regenerate, Runnable reload) {
        String[] args = message.trim().split("\\s+");
        if (!args[0].equals("/cbbg")) return false;
        if (args.length == 1 || args.length == 2 && args[1].equals("help")) {
            for (String key : new String[] {"header", "mode", "mode_set", "format", "format_set", "stbn", "notification"}) {
    ...
    private static void selection(String[] args, Feedback feedback) {
        ...
        if (kind.equals("mode")) {
            Mode mode;
            try { mode = Mode.valueOf(value.toUpperCase(Locale.ROOT)); }
            ...
            PixelFormat format;
            try { format = PixelFormat.valueOf(value.toUpperCase(Locale.ROOT)); }
            catch (IllegalArgumentException invalid) {
    ...
    private static Object name(String kind, Feedback feedback) {
        return kind.equals("mode")
                ...
                : feedback.translate("cbbg.pixel_format." + CbbgConfig.get().pixelFormat().getSerializedName());
    }

    private static void noise(String[] args, Feedback feedback, Runnable regenerate, Runnable reload) {
        if (args.length == 2) {
            CbbgConfig config = CbbgConfig.get();
            send(feedback, false, "stbn.usage.header", config.stbnSize(), config.stbnDepth(), config.stbnSeed());
        ...
        if (args.length == 3 && kind.equals("generate")) {
            ...
            if (args.length == 3) {
                CbbgConfig config = CbbgConfig.get();
                long value = kind.equals("size") ? config.stbnSize()
            ...
            if (args.length != 4) { usage(feedback); return; }
            long value;
    ...
    private static void notification(String[] args, boolean toast, Feedback feedback) {
        ...
        send(feedback, false, "notification." + kind + ".current",
