    ...
    public void runTest(ClientGameTestContext context) {
        for (GpuFormat format : new GpuFormat[] {GpuFormat.RGBA16_FLOAT, GpuFormat.RGBA32_FLOAT}) {
            ...
            context.runOnClient(client -> {
                ...
                var buffer = device.createBuffer(() -> "CBBG precision readback", 9, format.blockSize());
                try {
                    var encoder = device.createCommandEncoder();
                    encoder.clearColorTexture(source.getColorTexture(),
                            new Vector4f(1.0f / 1024, 3.0f / 1024, 5.0f / 1024, 1));
                    try (RenderPass pass = encoder.createRenderPass(() -> "CBBG precision",
                    ...
                    encoder.copyTextureToBuffer(target.getColorTexture(), buffer, 0, () -> {
                        try (var mapped = buffer.map(true, false)) {
                            ...
                            for (int i = 0; i < 4; i++) {
                                boolean exact = format == GpuFormat.RGBA16_FLOAT
                                        ? Short.toUnsignedInt(pixels.getShort(i * 2)) == halfBits[i]
                                        : pixels.getFloat(i * 4) == floats[i];
                                if (!exact) {
                                    throw new AssertionError(format + " lost float precision in channel " + i);
                                }
                            }
