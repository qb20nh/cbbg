    ...
    public void runTest(ClientGameTestContext context) {
        ...
        try {
            ...
            context.runOnClient(client -> {
                ...
                try {
                    chain.process(target, new GraphicsResourceAllocator() {
                        @Override
                        public <T> T acquire(ResourceDescriptor<T> descriptor) { throw injected; }
                        @Override
                ...
                var other = client.getShaderManager().getPostChain(
                ...
                try {
                    ...
                    MenuBlurScope.run(GpuFormat.RGBA32_FLOAT, () -> {
                        other.process(unrelated, new GraphicsResourceAllocator() {
                            @Override
                            public <T> T acquire(ResourceDescriptor<T> descriptor) {
                                if (MenuBlurScope.format() != null || !(descriptor instanceof RenderTargetDescriptor rt)
                                        || rt.color().format() != GpuFormat.RGBA8_UNORM) {
                                    throw new AssertionError("Unrelated post chain inherited the blur scope");
                                }
