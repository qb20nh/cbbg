...
public final class RenderScaleGameTest implements FabricClientGameTest {
        ...
        try (var world = context.worldBuilder().create()) {
            ...
            context.runOnClient(client -> {
                ...
                if (RenderScaleCompat.getDitherCoordScale() != 0.5f) {
                    throw new AssertionError("Dithering used the configured ceiling instead of dynamic scale");
                }
            });
            context.runOnClient(client -> {
    ...
    private static void check(ClientGameTestContext context, Object renderer, float coordinateScale,
            float renderScale, boolean fsr) {
        ...
        try {
            context.runOnClient(client -> {
                ...
                call(renderer, "blitAndBlendToTexture",
                        ...
                        scaled, main, Boolean.TRUE.equals(call(call(null, "getConfig"), "getFilter"))
                ...
                var buffer = device.createBuffer(() -> "RenderScale precision readback", 9,
                ...
                encoder.clearColorTexture(main.getColorTexture(),
                        new Vector4f(127.25f / 255, 127.25f / 255, 127.25f / 255, 1));
                capture(main, actual);
                DitherPass pass = new DitherPass();
                reference.set(pass);
                capture(pass.render(main.getColorTextureView(),
                        (GpuTextureView) field(DitherController.class, "noiseView"),
                        1, coordinateScale, coordinateScale, false), expected);
            });
