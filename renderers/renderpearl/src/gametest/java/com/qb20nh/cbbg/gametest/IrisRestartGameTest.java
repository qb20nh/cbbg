  ...
  public void runTest(ClientGameTestContext context) {
    ...
    context.runOnClient(
        client -> {
          ...
          if (!Objects.requireNonNull(phase).equals("verify")) {
            installPack();
            CbbgConfig.setMode(
                phase.equals("control") ? CbbgConfig.Mode.DISABLED : CbbgConfig.Mode.ENABLED);
            ...
            try {
              if (!Objects.requireNonNull(
                      (Boolean) config.getClass().getMethod("areShadersEnabled").invoke(config))
    ...
    try (var world = context.worldBuilder().create()) {
      ...
      context.waitFor(
          client -> IrisCompat.isShaderPackActive() && !DitherController.isReady(), 600);
      ...
      context.runOnClient(
          client -> {
            ...
            Screenshot.takeScreenshot(
                ...
                image -> {
                  try (image) {
                    assertFinalShader(image, "Persisted shader did not reach the screenshot");
                    Path evidence =
