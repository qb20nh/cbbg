...
public final class MainTargetsGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context) {
        ...
        TextureTarget custom = context.computeOnClient(client ->
                new TextureTarget("Other Mod / initFbo", 13, 7, GpuFormat.RGBA8_UNORM, GpuFormat.D32_FLOAT));
        ...
        LoggerFactory.getLogger("cbbg-test").info("MainTargets setup: allocate and destroy closed main target");
        MainTarget closed = context.computeOnClient(client -> {
        ...
        try {
            for (var mode : new CbbgConfig.Mode[] {CbbgConfig.Mode.ENABLED, CbbgConfig.Mode.DISABLED}) {
                for (var precision : new CbbgConfig.PixelFormat[] {
                    ...
                    LoggerFactory.getLogger("cbbg-test").info("MainTargets transition mode={} precision={}", mode, precision);
                    ...
                    context.waitTicks(3);
                    context.runOnClient(client -> {
                        GpuFormat expected = !mode.isActive() ? GpuFormat.RGBA8_UNORM
                                : precision == CbbgConfig.PixelFormat.RGBA32F
                                        ? GpuFormat.RGBA32_FLOAT : GpuFormat.RGBA16_FLOAT;
                        if (live.getColorTexture().getFormat() != expected || !previous.isClosed()
                                || live.width != 16 || live.height != 8) {
                            throw new AssertionError("Secondary main target did not follow the precision policy");
                        }
                        if (closed.getColorTexture() != null) {
                            throw new AssertionError("Precision change resurrected a destroyed target");
                        }
                        if (custom.getColorTexture() != customColor || custom.getDepthTexture() != customDepth
                                || customColor.getFormat() != GpuFormat.RGBA8_UNORM) {
                            throw new AssertionError("Precision change mutated a mod-owned framebuffer");
                        }
                    });
                    LoggerFactory.getLogger("cbbg-test").info("MainTargets transition verified mode={} precision={}", mode, precision);
                }
            }
            context.runOnClient(client -> {
                custom.resize(19, 11);
                if (!customColor.isClosed() || !customDepth.isClosed()
                        || custom.getColorTexture().getFormat() != GpuFormat.RGBA8_UNORM
                        || custom.getDepthTexture().getFormat() != customDepth.getFormat()) {
                    throw new AssertionError("Mod-owned framebuffer recreation changed attachment formats");
                }
            });
        } finally {
            context.runOnClient(client -> {
