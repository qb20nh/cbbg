  ...
  public void runTest(ClientGameTestContext context) {
    ...
    try {
      for (boolean fullscreen : new boolean[] {true, false, true, false}) {
        ...
        int[] nativeSize =
            context.computeOnClient(client -> pixelSize(client.getWindow().handle()));
        context.getInput().resizeWindow(nativeSize[0], nativeSize[1]);
        context.waitFor(
            client -> {
              var window = client.getWindow();
              boolean actual =
                  (SDLVideo.SDL_GetWindowFlags(window.handle()) & SDLVideo.SDL_WINDOW_FULLSCREEN)
                      != 0;
              var target = client.gameRenderer.mainRenderTarget();
              int[] pixels = pixelSize(window.handle());
              return actual == fullscreen
                  && window.getWidth() > 0
                  && window.getHeight() > 0
                  && window.getWidth() == pixels[0]
                  && window.getHeight() == pixels[1]
                  && target.width == window.getWidth()
                  && target.height == window.getHeight();
            },
            200);
        context.waitTicks(5);
                    ...
                    try (image) {
                      if (image.getWidth() != width || image.getHeight() != height) {
                        throw new AssertionError(
                            "Screenshot dimensions did not follow fullscreen transition");
                      }
