...
public final class MenuStrengthGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context) {
        ...
        try (var world = context.worldBuilder().create()) {
            ...
            Screen menu = context.computeOnClient(client ->
                    new Screen(Component.literal("Menu strength test")) {});
    ...
    private static void check(ClientGameTestContext context, Screen screen, int blur,
            float base, float expectedStrength) {
        ...
        try {
            context.runOnClient(client -> {
                ...
                RenderSystem.getDevice().createCommandEncoder().clearColorTexture(main.getColorTexture(),
                ...
                pass.set(reference);
                capture(reference.render(main.getColorTextureView(), noiseView(),
                        expectedStrength, 1, 1, false), expected);
                capture(reference.render(main.getColorTextureView(), noiseView(),
                        base, 1, 1, false), unboosted);
            });
