...
public final class DitherPassGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context) {
        Fixture fixture = context.computeOnClient(client -> {
            ...
            var source = new TextureTarget("CBBG dither fixture", WIDTH, HEIGHT, GpuFormat.RGBA32_FLOAT, null);
            device.createCommandEncoder().clearColorTexture(source.getColorTexture(),
                    new Vector4f(127.25f / 255, 127.25f / 255, 127.25f / 255, 0.375f));
            var noise = device.createTexture("CBBG test noise", 5, GpuFormat.RGBA8_UNORM, 2, 2, 1, 1);
        ...
        try {
            ...
            context.runOnClient(client -> {
                var old = fixture.pass.render(fixture.source.getColorTextureView(), fixture.view,
                        1, 1, 1, false).getColorTexture();
                var small = new TextureTarget("CBBG resize fixture", 3, 2, GpuFormat.RGBA32_FLOAT, null);
                try {
                    var resized = fixture.pass.render(small.getColorTextureView(), fixture.view,
                            1, 1, 1, false);
                    if (!old.isClosed() || resized.width != 3 || resized.height != 2) {
    ...
    private static void check(ClientGameTestContext context, Fixture fixture, boolean demo,
            float strength, float scale) {
        CompletableFuture<Void> result = new CompletableFuture<>();
        context.runOnClient(client -> {
            var output = fixture.pass.render(fixture.source.getColorTextureView(), fixture.view,
                    strength, scale, scale, demo);
            if (output.getColorTexture().getFormat() != GpuFormat.RGBA8_UNORM) {
                throw new AssertionError("Dither output must be RGBA8");
            }
            Screenshot.takeScreenshot(output, image -> {
                try (image) {
                        ...
                        for (int x = 0; x < WIDTH; x++) {
                            ...
                            if (image.getPixel(x, y) != expected) {
                                throw new AssertionError("Dither mismatch at " + x + "," + y
                                        + " demo=" + demo + " strength=" + strength + " scale=" + scale
    ...
    private record Fixture(TextureTarget source, GpuTexture noise, GpuTextureView view,
            DitherPass pass) implements AutoCloseable {
        @Override
        public void close() {
            pass.close();
            view.close();
