  ...
  public void runTest(ClientGameTestContext context) {
    ...
    if (!Float.isFinite(strength) || strength <= 0) {
      throw new AssertionError(
          "RenderScale pixel-grid reference requires positive finite strength");
    }
        ...
        context.computeOnClient(
            client -> {
              ...
              encoder.copyTextureToBuffer(
                      ...
                      for (int i = 0; i < 4; i++) {
                        float value =
                            format == GpuFormat.RGBA16_FLOAT
                                ...
                                : bytes.getFloat(i * 4);
                        // Preserve the original FSR bound: over 100 times smaller than an 8-bit
                        // step.
                  ...
                  0);
              encoder.clearColorTexture(
                  main.getColorTexture(), new Vector4f(SOURCE, SOURCE, SOURCE, 1));
              ...
              if (ProcessedRenderObservations.draws() != draws + 1) {
                throw new AssertionError(
                    "RenderScale screenshot did not execute the packaged dither pass");
              }
      ...
      for (int y = 0; y < sample.height(); y++) {
        for (int x = 0; x < sample.width(); x++) {
          if (actual[y * sample.width() + x] != expected.getPixel(x, y)) {
            throw new AssertionError(
                "RenderScale screenshot used the wrong dither pixel grid at "
                    + x
                    + ","
                    + y
                    + " scenario="
                    + scenario
                    + " expected="
                    + Integer.toHexString(expected.getPixel(x, y))
                    + " actual="
    ...
    for (int channel = 0; channel < 3; channel++) {
      ...
      pixel |=
          DitherReference.channel(
                  source, noisePixel >>> shift & 255, strength, x, sample.width(), false)
              << shift;
    }
    return pixel;
  }
