...
public final class ReleaseNotificationsGameTest implements FabricClientGameTest {
        ...
        try (GenerationControl control = new GenerationControl()) {
            ...
            try (var world = context.worldBuilder().create()) {
                ...
                try {
                    ...
                    context.waitTicks(3);
                    context.runOnClient(client -> {
                        if (!old.interrupted) throw new AssertionError("Replacement did not cancel the old worker");
                    ...
                    context.runOnClient(client -> {
                        if (!followed.interrupted) throw new AssertionError("Implicit load did not cancel the old worker");
                        checkMessages(client, 1, 0);
                    });
                    implicit.release();
    ...
    private static void awaitCompletion(ClientGameTestContext context, long seed,
            boolean chat, boolean toast, int starts) {
        ReleaseClient.awaitCache(context, 16, 8, seed);
        if (chat || toast) {
            context.waitFor(client -> chat
                    ? generationMessages(client).stream().anyMatch(text -> text.endsWith(
                    ...
                    : normalized(Component.translatable("cbbg.toast.stbn.complete").getString())
                            .equals(toastText(client)), WAIT_TICKS);
        } else {
    ...
    private static void preferences(CommandDispatcher<FabricClientCommandSource> dispatcher,
            FabricClientCommandSource source, boolean chat, boolean toast) {
    ...
    private static void restore(ClientGameTestContext context,
            CommandDispatcher<FabricClientCommandSource> dispatcher,
            FabricClientCommandSource source, JsonObject original) {
        context.runOnClient(client -> {
            execute(dispatcher, source, "mode set disabled");
            ...
            preferences(dispatcher, source, original.get("notifyChat").getAsBoolean(),
                    original.get("notifyToast").getAsBoolean());
            execute(dispatcher, source, "mode set " + original.get("mode").getAsString().toLowerCase(Locale.ROOT));
        });
        context.waitFor(client -> original.equals(ReleaseClient.settings()), WAIT_TICKS);
    }

    private static void execute(CommandDispatcher<FabricClientCommandSource> dispatcher,
            FabricClientCommandSource source, String suffix) {
        try {
    ...
    private static void checkMessages(Minecraft client, int starts, int completions) {
        ...
        long actualStarts = texts.stream().filter(text -> text.endsWith(
                Component.translatable("cbbg.chat.stbn.generating").getString())).count();
        long actualCompletions = texts.stream().filter(text -> text.endsWith(
                Component.translatable("cbbg.chat.stbn.complete").getString())).count();
        if (actualStarts != starts || actualCompletions != completions) {
            throw new AssertionError("Expected generation messages " + starts + "/" + completions + ", got " + texts);
        }
    }

    @SuppressWarnings("unchecked")
    private static List<String> generationMessages(Minecraft client) {
        List<GuiMessage> messages = (List<GuiMessage>) field(ChatComponent.class,
                "allMessages", client.gui.hud.getChat());
    ...
    private static void checkToast(Minecraft client, String state) {
        ...
        String expected = state == null ? null
    ...
    private static String toastText(Minecraft client) {
        SystemToast toast = client.gui.toastManager().getToast(SystemToast.class,
                SystemToast.SystemToastId.PERIODIC_NOTIFICATION);
        if (toast == null) return null;
        List<FormattedCharSequence> lines = (List<FormattedCharSequence>) field(
                SystemToast.class, "messageLines", toast);
    ...
    private static final class Gate {
        ...
        private Gate(boolean fail) { this.fail = fail; }
    ...
    private static final class GenerationControl extends AbstractAppender implements AutoCloseable {
        ...
        private Gate arm(boolean fail) {
            ...
            gates.add(gate);
            if (!next.compareAndSet(null, gate)) throw new AssertionError("Generation gate already armed");
            return gate;
        }

        @Override
        public void append(LogEvent event) {
            if (!event.getThreadName().equals("cbbg-stbn") || !event.getMessage().getFormattedMessage()
