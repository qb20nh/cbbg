...
public final class ReleaseMaximumNoiseCacheGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context) {
        ...
        try (var world = context.worldBuilder().create()) {
            ...
            String debug = context.computeOnClient(client -> String.join(" ", ReleaseDebugState.read(client)));
    ...
    private static void disable(ClientGameTestContext context) {
        GpuTextureView noise = context.computeOnClient(client -> ProcessedRenderObservations.lastDitherNoise());
        GpuTextureView output = context.computeOnClient(client -> ProcessedRenderObservations.lastDitherOutput());
        ReleaseClient.command(context, "mode set disabled");
        context.waitFor(client -> "disabled".equalsIgnoreCase(ReleaseClient.settings().get("mode").getAsString())
                && (noise == null || noise.texture().isClosed())
                && (output == null || output.texture().isClosed()), WAIT_TICKS);
        ReleaseClient.assertNoDraws(context);
    }

    private static void enable(ClientGameTestContext context, int timeout) {
        GpuTextureView old = context.computeOnClient(client -> ProcessedRenderObservations.lastDitherNoise());
        ReleaseClient.command(context, "mode set enabled");
        context.waitFor(client -> "enabled".equalsIgnoreCase(ReleaseClient.settings().get("mode").getAsString()),
                WAIT_TICKS);
    ...
    private static void awaitNoise(ClientGameTestContext context, GpuTextureView old, int timeout) {
        ...
        context.waitFor(client -> {
            ...
            return client.gui.overlay() == null && noise != null && noise != old && !noise.texture().isClosed()
    ...
    private static int[][] readCache() throws Exception {
        ...
        for (int z = 0; z < DEPTH; z++) {
            ...
            if (entry.length != 2 || !entry[0].equals(hash)
                    || !entry[1].equals(path(z).getFileName().toString())) {
                throw new AssertionError("Incorrect maximum-size PNG hash for frame " + z);
            }
            try (var input = new java.io.ByteArrayInputStream(png); NativeImage image = NativeImage.read(input)) {
                ...
                for (int y = 0; y < SIZE; y++) {
                    for (int x = 0; x < SIZE; x++) {
                        ...
                        for (int channel = 0; channel < 4; channel++) pixel[channel] = (byte) (value >>> (channel * 8));
    ...
    private static void assertCycle(ClientGameTestContext context, int[][] expected) {
        ...
        try {
            context.runOnClient(client -> ProcessedRenderObservations.setNoiseObserver(noise -> {
            ...
            context.waitFor(client -> samples.stream().allMatch(sample -> sample.pixels().isDone()), WAIT_TICKS);
        } finally {
            ...
            for (int y = 0; y < SIZE; y++) {
                for (int x = 0; x < SIZE; x++) {
                    ...
                    if (actual[y * SIZE + x] != expected[next.frame()][(SIZE - 1 - y) * SIZE + x]) {
                        throw new AssertionError("Maximum-size GPU noise differs from cache frame "
                                + next.frame() + " at " + x + "," + y);
                    }
                }
            }
        ...
        if (samples.size() != DEPTH + 1) throw new AssertionError("Incomplete maximum-size actual GPU cycle");
    ...
    private static CompletableFuture<int[]> readNoise(GpuTextureView noise) {
        ...
        TextureTarget target = new TextureTarget("CBBG maximum noise probe", SIZE, SIZE, GpuFormat.RGBA8_UNORM, null);
        try {
            try (RenderPass pass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(
                    () -> "CBBG maximum noise probe", target.getColorTextureView(), Optional.empty())) {
    ...
    private static Path path(int frame) {
        return ReleaseClient.cache().resolve("stbn_256x256x128"
                + (frame < 0 ? ".sha256" : "_" + frame + ".png"));
    }
    ...
    private static String logSince(long offset) throws Exception {
        if (Files.size(log()) < offset) throw new AssertionError("Client log rotated during cache observation");
