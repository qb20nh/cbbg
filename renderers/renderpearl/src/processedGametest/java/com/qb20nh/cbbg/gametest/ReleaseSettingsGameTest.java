...
public final class ReleaseSettingsGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context) {
        ...
        if (installed) {
            ...
            check(version.equals(System.getProperty("cbbg.test.modmenu.version")),
    ...
    private static final class Installed {
        static void run(ClientGameTestContext context) {
            ...
            try (var world = context.worldBuilder().create()) {
                ...
                try {
                    ...
                    open(context, mods);
                    GpuTextureView oldNoise = context.computeOnClient(client ->
                    ...
                    context.waitFor(client -> {
                        ...
                        return noise != null && noise != oldNoise && !noise.texture().isClosed()
                                && noise.getWidth(0) == 16 && noise.getHeight(0) == 16
                    ...
                    check(edited.get("strength").getAsFloat() == 4f
                            && !edited.get("notifyChat").getAsBoolean()
                            && !edited.get("notifyToast").getAsBoolean(), "Widget edits were not saved");
                    open(context, mods);
                    context.runOnClient(client -> {
                        var ui = ReleaseSettingsUi.from(client.gui.screen());
                        ...
                        check(ReleaseClient.settings().get("stbnSeed").getAsLong() == 123,
                    ...
                    context.runOnClient(client -> {
                        check(client.gui.screen() == mods && mods.modScreenErrors.isEmpty(),
        ...
        private static void restore(ClientGameTestContext context,
                com.terraformersmc.modmenu.gui.ModsScreen mods, JsonObject original) {
            command(context, "mode set enabled");
            open(context, mods);
            ReleaseSettingsUi.setStrength(context, original.get("strength").getAsFloat());
            command(context, "mode set disabled");
            command(context, "format set " + original.get("pixelFormat").getAsString().toLowerCase(Locale.ROOT));
        ...
        private static void locales(ClientGameTestContext context,
                com.terraformersmc.modmenu.gui.ModsScreen mods) {
            ...
            Map<String, Map<String, String>> locales = context.computeOnClient(client -> {
                ...
                client.getResourceManager().listResources("lang", id -> id.getNamespace().equals("cbbg")
                    ...
                    check(!expected.isEmpty() && client.getLanguageManager().getLanguage(locale) != null,
                            "Empty or unavailable locale " + locale);
                    result.put(locale, expected);
                });
                check(result.containsKey("en_us") && result.size() > 1, "Packaged translations missing");
                return result;
            });
            try {
                for (var entry : locales.entrySet()) {
                    check(entry.getValue().keySet().equals(locales.get("en_us").keySet()),
                            "Locale keys differ from English: " + entry.getKey());
                    reload(context, entry.getKey());
                    context.runOnClient(client -> entry.getValue().forEach((key, value) ->
                            check(value.equals(Language.getInstance().getOrDefault(key)),
    ...
    private static void editNoise(Screen screen) {
        ...
        for (String invalid : List.of("-", "9223372036854775808", "-9223372036854775809")) {
            ...
            check(net.minecraft.client.Minecraft.getInstance().gui.screen() == screen,
                    "Invalid seed opened generation confirmation");
        }
        ui.seed().setValue("123");
    }

    private static void retainNoise(ClientGameTestContext context, GpuTextureView noise, String action) {
        long before = ProcessedRenderObservations.draws();
        ReleaseClient.awaitDrawAfter(context, before);
        context.waitTicks(5);
        context.runOnClient(client -> check(ProcessedRenderObservations.lastDitherNoise() == noise
                && !noise.texture().isClosed(), "GPU noise changed while " + action));
    }
