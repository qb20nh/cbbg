...
public final class ReleasePersistentBlurGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context) {
        ...
        try (var world = context.worldBuilder().create()) {
            ...
            try {
                for (GpuFormat format : new GpuFormat[] {GpuFormat.RGBA16_FLOAT,
                        GpuFormat.RGBA32_FLOAT, GpuFormat.RGBA16_FLOAT, GpuFormat.RGBA8_UNORM}) {
                    ReleaseClient.command(context, "mode set "
                            + (format == GpuFormat.RGBA8_UNORM ? "disabled" : "enabled"));
                    ReleaseClient.command(context, "format set "
                            + (format == GpuFormat.RGBA32_FLOAT ? "rgba32f" : "rgba16f"));
                    ...
                    context.runOnClient(client -> {
                        ...
                        if (current == null || current.getColorTexture().getFormat() != format
                                || current.width != main.width || current.height != main.height) {
                            throw new AssertionError("Persistent blur target has the wrong format or dimensions");
                        }
    ...
    private static PostChain createChain(Minecraft client) {
        ...
        try (var reader = client.getResourceManager().getResourceOrThrow(
                Identifier.withDefaultNamespace("post_effect/blur.json")).openAsReader()) {
            ...
            return PostChain.load(config, client.getTextureManager(), LevelTargetBundle.MAIN_TARGETS,
                    id, (Projection) field(vanilla, "projection"),
