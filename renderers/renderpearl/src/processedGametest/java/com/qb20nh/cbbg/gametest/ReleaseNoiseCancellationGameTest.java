...
public final class ReleaseNoiseCancellationGameTest implements FabricClientGameTest {
        ...
        try (var world = context.worldBuilder().create()) {
            ...
            try (GenerationObserver observer = new GenerationObserver()) {
                try {
                    ...
                    context.waitFor(client -> observer.replacementStarted && observer.workerIdle()
                    ...
                    ReleaseClient.assertSettings("ENABLED", original.get("pixelFormat").getAsString(),
                            16, 8, REPLACEMENT_SEED);
                    assertReplacementPixels();
                    if (!observer.workerIdle() || observer.completions != 1
                            || observer.replacementWorker != observer.worker) {
                        throw new AssertionError("Obsolete math completed or worker still runs after replacement");
                    }
                    completed = true;
                    ...
                    try {
                        ...
                        context.runOnClient(client -> {
                            execute(dispatcher, source, "mode set disabled");
                            configure(dispatcher, source, original.get("stbnSize").getAsInt(),
                                    original.get("stbnDepth").getAsInt(), original.get("stbnSeed").getAsLong());
                            execute(dispatcher, source, "mode set "
    ...
    private static void configure(CommandDispatcher<FabricClientCommandSource> dispatcher,
            FabricClientCommandSource source, int size, int depth, long seed) {
    ...
    private static void assertReplacementPixels() throws Exception {
        ...
        ByteBuffer pixels = ByteBuffer.allocate(16 * 16 * 8 * Integer.BYTES).order(ByteOrder.BIG_ENDIAN);
        for (int z = 0; z < 8; z++) {
    ...
    private static final class GenerationObserver extends AbstractAppender implements AutoCloseable {
        ...
        public void append(LogEvent event) {
            ...
            } else if (worker != null && message.startsWith("Starting Async STBN Math Generation (16x16x8)")) {
