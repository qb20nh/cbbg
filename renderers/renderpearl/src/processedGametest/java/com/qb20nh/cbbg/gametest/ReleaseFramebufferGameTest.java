...
public final class ReleaseFramebufferGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context) {
        ...
        try (var world = context.worldBuilder().create()) {
            ...
            context.waitTicks(3);
            TextureTarget custom = context.computeOnClient(client -> new TextureTarget(
            ...
            try {
                for (String mode : new String[] {"enabled", "disabled"}) {
                    for (String precision : new String[] {"rgba32f", "rgba16f"}) {
                        ...
                        GpuFormat expected = mode.equals("disabled") ? GpuFormat.RGBA8_UNORM
                        ...
                        context.runOnClient(client -> {
                            if (!previous.isClosed() || live.width != 16 || live.height != 8) {
                                throw new AssertionError("Secondary main target retained its old attachment or changed size");
                            }
                            ...
                            if (!old.isClosed() || main.getDepthTexture() == null
                                    || main.getColorTexture().getFormat() != expected) {
                                throw new AssertionError("Main target resize failed to retain the selected format");
                            }
                        });
                    }
                }
                ...
                ReleaseClient.command(context, "mode set "
                        + original.get("mode").getAsString().toLowerCase(java.util.Locale.ROOT));
                context.waitTicks(3);
            }
        }
    }
