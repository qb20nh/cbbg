...
public final class ReleaseNoiseCacheGameTest implements FabricClientGameTest {
    ...
    // CPU reference: BlueNoise.generateScalarField U/V fields and calculatePixelColor, compiled offline.
    ...
    private static void disable(ClientGameTestContext context) {
        ...
        context.waitFor(client -> "disabled".equalsIgnoreCase(
                ReleaseClient.settings().get("mode").getAsString())
    ...
    private static int[] load(ClientGameTestContext context, long seed) throws Exception {
        ...
        context.waitFor(client -> "enabled".equalsIgnoreCase(
    ...
    private static void awaitNoise(ClientGameTestContext context, GpuTextureView old) {
        ...
        context.waitFor(client -> {
            ...
            return noise != null && noise != old && !noise.texture().isClosed()
    ...
    private static Path path(int frame) {
        return ReleaseClient.cache().resolve(frame < 0 ? "stbn_16x16x8.sha256"
                : "stbn_16x16x8_" + frame + ".png");
    }
    ...
    private static int[] readCache(long seed) throws Exception {
        ...
        for (int z = 0; z < DEPTH; z++) {
            ...
            if (entry.length != 2 || !entry[0].equals(hash(bytes))
