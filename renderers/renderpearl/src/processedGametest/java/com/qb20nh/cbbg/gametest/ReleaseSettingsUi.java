...
record ReleaseSettingsUi(CycleButton<?> mode, CycleButton<?> format,
        AbstractSliderButton strength, AbstractSliderButton size, AbstractSliderButton depth,
        EditBox seed, Button generate, CycleButton<?> chat, CycleButton<?> toast, Button done) {
    ...
    static void setStrength(ClientGameTestContext context, float strength) {
        context.runOnClient(client -> slide(from(client.gui.screen()).strength(),
                (strength - 0.5) / 3.5));
    ...
    private static final class Installed {
        static Screen mods(ClientGameTestContext context, Screen parent) {
            return context.computeOnClient(client -> new com.terraformersmc.modmenu.gui.ModsScreen(parent));
        }
    ...
    static ReleaseSettingsUi from(Screen screen) {
        ...
        return new ReleaseSettingsUi(find(screen, CycleButton.class, "cbbg.config.mode"),
    ...
    private static <T extends AbstractWidget> T find(Screen screen, Class<T> type, String key) {
        ...
        var matches = screen.children().stream().filter(type::isInstance).map(type::cast)
                .filter(widget -> widget.getMessage().getString().contains(label)).toList();
    ...
    private static void clickAt(AbstractWidget widget, double x) {
        widget.onClick(new MouseButtonEvent(x, widget.getY() + widget.getHeight() / 2.0,
