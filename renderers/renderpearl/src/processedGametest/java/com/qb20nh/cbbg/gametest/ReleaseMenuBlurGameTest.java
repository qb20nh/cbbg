...
public final class ReleaseMenuBlurGameTest implements FabricClientGameTest {
  ...
  private static void setMode(ClientGameTestContext context, String mode) {
    ...
    context.waitFor(
        client -> mode.equalsIgnoreCase(ReleaseClient.settings().get("mode").getAsString()), 600);
    context.waitTicks(3);
  }
  ...
  private static PostChain chain(net.minecraft.client.Minecraft client, String name) {
    var chain =
        client
            .getShaderManager()
            .getPostChain(Identifier.withDefaultNamespace(name), LevelTargetBundle.MAIN_TARGETS);
    if (chain == null) {
  ...
  private static void assertFormat(ResourceDescriptor<?> descriptor, GpuFormat expected) {
    if (!(descriptor instanceof RenderTargetDescriptor rt)
      ...
      throw new AssertionError(
          "Expected post-chain descriptor " + expected + ", got " + descriptor);
    }
  }
        ...
        client -> {
          ...
          try {
            ...
            try {
              ...
              if (acquisitions[0] == 0)
                throw new AssertionError("Nested blur did not allocate targets");
              probeScope(blur, target, GpuFormat.RGBA8_UNORM);
            } finally {
  ...
  private static void processUnrelated(
      PostChain other, PostChain blur, RenderTarget unrelated, RenderTarget probeTarget) {
    int[] acquisitions = {0};
    other.process(
        unrelated,
        new GraphicsResourceAllocator() {
  ...
  private static void check(
    ...
    context.runOnClient(
        client -> {
          ...
          try {
            var buffer =
                device.createBuffer(() -> "CBBG blur readback", 9, 16L * format.blockSize());
