...
public final class ReleaseImmediatelyFastGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context) {
        ...
        if (iris != requested.contains("iris")) {
            throw new AssertionError("Iris presence differs from the requested fixture");
        }
        if (!expected) return;
        ...
        try (var world = context.worldBuilder().create()) {
            ...
            try {
                ...
                for (String mode : new String[] {"ENABLED", "DISABLED", "DEMO"}) {
                    for (String format : new String[] {"RGBA16F", "RGBA32F"}) {
                        setSettings(context, mode, format);
                        context.runOnClient(client -> {
                            if (atlas.getColorTexture() != color || atlas.getDepthTexture() != depth
                ...
                if (iris) checkShaderTransitions(context, atlas);
                context.runOnClient(client -> {
                    close(atlas);
                    owned.remove(atlas);
                ...
                try {
                    ...
                    setSettings(context, original.get("mode").getAsString(),
    ...
    private static void setSettings(ClientGameTestContext context, String mode, String format) {
        ReleaseClient.command(context, "mode set " + mode.toLowerCase(Locale.ROOT));
        ReleaseClient.command(context, "format set " + format.toLowerCase(Locale.ROOT));
        context.waitFor(client -> {
            JsonObject saved = ReleaseClient.settings();
            return mode.equals(saved.get("mode").getAsString())
    ...
    private static void checkShaderTransitions(ClientGameTestContext context, RenderTarget atlas) {
        ...
        var depth = context.computeOnClient(client -> atlas.getDepthTexture());
        try {
            setSettings(context, "ENABLED", ReleaseClient.settings().get("pixelFormat").getAsString());
            for (boolean active : new boolean[] {true, false}) {
                ...
                context.waitFor(client -> shaderPackActive() == active, WAIT_TICKS);
                context.runOnClient(client -> {
                    if (atlas.getColorTexture() != color || atlas.getDepthTexture() != depth
                            || color.isClosed() || depth.isClosed()) {
            ...
            context.runOnClient(client -> {
                IrisFixture.invoke(IrisFixture.iris("getIrisConfig"), "setShadersEnabled", boolean.class, false);
                IrisFixture.saveAndReload();
            });
    ...
    private static RenderTarget create(List<RenderTarget> owned) {
        try {
            RenderTarget atlas = (RenderTarget) Class.forName(ATLAS)
                    .getConstructor(int.class).newInstance(TEST_ATLAS_ID);
            owned.add(atlas);
            return atlas;
        } catch (ReflectiveOperationException failure) {
    ...
    private static void checkClearReadback(ClientGameTestContext context, RenderTarget atlas) {
        ...
        context.runOnClient(client -> Screenshot.takeScreenshot(atlas, Math.max(1, atlas.width / 16), image -> {
            try (image) {
                // Vanilla screenshots force opaque alpha, even for a transparent clear.
                for (int y = 0; y < image.getHeight(); y++) {
                    for (int x = 0; x < image.getWidth(); x++) {
                        if (image.getPixel(x, y) != 0xFF000000) {
                            throw new AssertionError("ImmediatelyFast atlas clear/readback was altered at "
                                    + x + "," + y);
                        }
                    }
                }
