...
public final class ReleaseWorldPixelsGameTest implements FabricClientGameTest {
    ...
    static void runScene(ClientGameTestContext context, boolean disabledControl) {
        ...
        try (var world = context.worldBuilder().create()) {
            ...
            try {
                ...
                ReleaseClient.awaitFormat(context, disabledControl
                        ? GpuFormat.RGBA8_UNORM : GpuFormat.RGBA32_FLOAT);
                ReleaseClient.assertSettings(disabledControl ? "DISABLED" : "ENABLED",
                        "RGBA32F", NOISE_SIZE, NOISE_DEPTH, NOISE_SEED);
    ...
    private static void capture(ClientGameTestContext context, boolean demo) {
        ...
        ReleaseClient.assertSettings(demo ? "DEMO" : "ENABLED",
                "RGBA32F", NOISE_SIZE, NOISE_DEPTH, NOISE_SEED);
        ...
        context.runOnClient(client -> {
            try {
                Path noisePath = ReleaseClient.cache().resolve("stbn_"
                        + NOISE_SIZE + "x" + NOISE_SIZE + "x" + NOISE_DEPTH + "_0.png");
                ...
                try (noiseImage) {
                    ...
                    noiseTexture = device.createTexture("CBBG world reference noise", 5,
                            GpuFormat.RGBA8_UNORM, tileSize, tileSize, 1, 1);
                    try {
                ...
                try {
                    ...
                    Files.createDirectories(directory);
                    Files.copy(noisePath, directory.resolve("noise.png"),
