...
public final class ReleaseSulkanRestartGameTest implements FabricClientGameTest {
    ...
    static void run(ClientGameTestContext context, String pack) {
        ...
        context.runOnClient(client -> {
            ReleaseSulkanGameTest.checkGate(client, ReleaseSulkanGameTest.userMode(client),
                    ReleaseSulkanGameTest.active());
        ...
        try (var world = context.worldBuilder().create()) {
            ...
            if (!phase.equals("verify")) {
                ...
                try {
                    ...
                    Files.writeString(evidence.resolve("sulkan-restart-original-settings.json"), original + "\n");
                } catch (java.io.IOException failure) {
                    throw new AssertionError("Could not retain pre-restart preferences", failure);
                }
                ReleaseClient.command(context, "mode set " + (phase.equals("control") ? "disabled" : "demo"));
            ...
            if (phase.equals("verify")) {
                ...
                if (ProcessedRenderObservations.draws() <= stoppedDraws
                    ...
                    throw new AssertionError("Disabling Sulkan after restart did not restore CBBG presentation");
                }
                if (!pack.equals("__builtin__")) {
