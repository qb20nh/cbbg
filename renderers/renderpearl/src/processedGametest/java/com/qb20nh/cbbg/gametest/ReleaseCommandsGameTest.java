...
public final class ReleaseCommandsGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context) {
        ...
        try (var world = context.worldBuilder().create()) {
            ...
            try {
                ...
                GpuTexture oldColor = context.computeOnClient(client -> {
                ...
                ReleaseClient.awaitDrawAfter(context, ProcessedRenderObservations.draws());
                long beforeGeneration = context.computeOnClient(client -> {
                    if (!oldColor.isClosed()) {
                        throw new AssertionError("Format replacement retained the old color attachment");
                    }
                    GpuTextureView oldNoiseBeforeGenerate = ProcessedRenderObservations.lastDitherNoise();
                    if (oldNoiseBeforeGenerate == null || oldNoiseBeforeGenerate.texture().isClosed()) {
    ...
    private static void restore(ClientGameTestContext context, FabricClientCommandSource source,
            JsonObject original) {
        context.runOnClient(client -> {
            execute(source, "mode set disabled", 1);
            execute(source, "format set " + original.get("pixelFormat").getAsString()
            ...
            execute(source, "mode set " + original.get("mode").getAsString().toLowerCase(Locale.ROOT), 1);
        });
        String expectedFormat = original.get("pixelFormat").getAsString();
        GpuFormat gpuFormat = switch (expectedFormat) {
