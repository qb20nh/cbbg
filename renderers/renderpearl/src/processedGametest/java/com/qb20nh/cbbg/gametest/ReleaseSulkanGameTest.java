...
public final class ReleaseSulkanGameTest implements FabricClientGameTest {
    ...
    static boolean vulkan(ClientGameTestContext context) {
        return context.computeOnClient(client -> "vulkan".equalsIgnoreCase(
    ...
    static void checkDebug(Minecraft client, String user, boolean suspended) {
        ...
        boolean frames = frame.find() && (stopped
                ? frame.group(1).equals("0") && frame.group(2).equals("0")
                : Integer.parseInt(frame.group(2)) == ReleaseClient.settings().get("stbnDepth").getAsInt()
        ...
        if ((saved.has("mode") && !saved.get("mode").getAsString().equals(user))
                || !frames || !output.contains("main=" + format.name())
        ...
        try {
            ...
            Files.writeString(evidence.resolve("sulkan-" + suspended + "-" + user + ".txt"), output + "\n");
    ...
    static void awaitRendering(ClientGameTestContext context, String user) {
        ...
        long draws = ProcessedRenderObservations.draws();
        long presentations = ProcessedRenderObservations.presentations();
        context.waitFor(client -> ProcessedRenderObservations.draws() > draws
                && ProcessedRenderObservations.presentations() > presentations, 600);
        context.runOnClient(client -> {
    ...
    static void restoreSettings(ClientGameTestContext context, JsonObject saved) {
        ...
        ReleaseClient.command(context, "format set " + saved.get("pixelFormat").getAsString().toLowerCase(Locale.ROOT));
        ReleaseClient.command(context, "mode set " + saved.get("mode").getAsString().toLowerCase(Locale.ROOT));
    ...
    static void select(ClientGameTestContext context, boolean enabled, String pack) {
        CompletableFuture<?> reload = context.computeOnClient(client -> (CompletableFuture<?>) invoke(
                "applySelection", new Class<?>[] {Minecraft.class, boolean.class, String.class}, client, enabled, pack));
    ...
    static void restoreConfig(ClientGameTestContext context, Object original) {
        CompletableFuture<?> restore = context.computeOnClient(client -> (CompletableFuture<?>) invoke(
                "applyConfig", new Class<?>[] {Minecraft.class, original.getClass(), boolean.class}, client, original, false));
