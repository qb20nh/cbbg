...
public final class PipelineFormats extends RenderPipeline {
  private PipelineFormats(RenderPipeline source, int attachment, GpuFormat format) {
    super(
        ...
        replace(source, attachment, format),
  ...
  public static RenderPipeline withColorFormat(
      RenderPipeline source, int attachment, GpuFormat format) {
  ...
  private static ColorTargetState[] replace(
      RenderPipeline source, int attachment, GpuFormat format) {
    ...
    states[attachment] =
        new ColorTargetState(original.blendFunction(), format, original.writeMask());
    return states;
  }
}
