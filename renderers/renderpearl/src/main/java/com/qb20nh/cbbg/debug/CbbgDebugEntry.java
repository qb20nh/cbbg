...
public final class CbbgDebugEntry implements DebugScreenEntry {
    ...
    private static String framebufferState(String backend) {
        ...
        try {
            ...
            int attachment = GL11.glGetBoolean(GL11.GL_DOUBLEBUFFER) ? GL11.GL_BACK_LEFT : GL11.GL_FRONT_LEFT;
            encoding = GL30.glGetFramebufferAttachmentParameteri(GL30.GL_DRAW_FRAMEBUFFER,
                    attachment, GL30.GL_FRAMEBUFFER_ATTACHMENT_COLOR_ENCODING);
        } finally {
            GL30.glBindFramebuffer(GL30.GL_DRAW_FRAMEBUFFER, previous);
        }
        String name = encoding == GL21.GL_SRGB ? "SRGB"
