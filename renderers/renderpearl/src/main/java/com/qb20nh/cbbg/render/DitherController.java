...
public final class DitherController {
    ...
    private static void startLoading(CbbgConfig settings) {
        ...
        loading = STBNGenerator.generateAsync(key.size, key.size, key.depth, key.seed)
                .thenApplyAsync(fields -> STBNLoader.loadOrGenerate(requested.size,
                        requested.size, requested.depth, fields));
    ...
    public static void beginFrame() {
        ...
        if (previousSettings != null && (previousMode != mode
                || previousSettings.pixelFormat() != settings.pixelFormat())) {
    ...
    private static TextureTarget render(GpuTextureView input, boolean advance) {
        if (failed || !CbbgClient.isEnabled()
        ...
        try {
            if (loading != null) {
                ...
                noise = RenderSystem.getDevice().createTexture("CBBG STBN", 5,
                        GpuFormat.RGBA8_UNORM, key.size, key.size, 1, 1);
            ...
            float strength = DitherStrength.effective(settings.strength(),
                    screen != null && !screen.isInGameUi(), client.options.getMenuBackgroundBlurriness());
            float coordScale = RenderScaleCompat.getDitherCoordScale();
            TextureTarget output = PASS.render(input, noiseView, strength, coordScale, coordScale,
    ...
    public static boolean hasDetectedNoFloatFormats() {
        ...
        return CbbgClient.isEnabled() && color != null
                && color.getFormat() == GpuFormat.RGBA8_UNORM;
    }
