...
public final class PersistentBlurGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context) {
        ...
        try {
            for (String format : new String[] {"RGBA16_FLOAT", "RGBA32_FLOAT", "RGBA16_FLOAT", "RGBA8_UNORM"}) {
                context.runOnClient(client -> {
                    CbbgConfig.setMode(format.equals("RGBA8_UNORM")
                            ? CbbgConfig.Mode.DISABLED : CbbgConfig.Mode.ENABLED);
                    CbbgConfig.setPixelFormat(format.equals("RGBA32_FLOAT")
                ...
                context.runOnClient(client -> {
                    ...
                    if (current == null || !current.getColorTexture().getFormat().name().equals(format)
